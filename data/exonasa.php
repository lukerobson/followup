...
for ($m=359; $m<$cont;$m++){ # for each line of the CSV file skiping lines 0-149 as there are comments in the CSV file on those lines
	...
	$period = $pass[6]; # gets the period in days
	$transittime = $pass[128]; # get transit time in day
	$atransitmid = dateare((($pass[132])-($transittime/2))); # gets the midtime in JD and turns the time to unixtime at start time of transit
	$transittime = ($pass[128])*24*60;# gets transit time in days turns to mins
	$ra = $pass[40]; # Right assension
	$dec = $pass[42]; # Declanation
	$depth = $pass[124]/100; # transit depth from % to magnitude
	$filterV = $pass[257]; # Magnitude in V- band johonson
